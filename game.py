...
def initializeRooms():
    ...
    rooms = {"kitchen":kitchen, "porch":porch, "entry way":entryway, "dining room":diningroom, "living room":livingroom,"bathroom":bathroom,"bedroom":bedroom,"secret room":secretroom}
    return rooms
...
def roomDescription(room):
    ...
    if room == "entry way":
        ...
        printNow("You hear footsteps to the east")
        printNow("There is a passageway to the north (kitchen), a door to the east (living room)")
    ...
    if room == "secret room":
        ...
        printNow("You look around, find a chest, and open the chest")
...
def openCloset():
    ...
    if "knife" in inventory: 
        ...
        printNow("You can go east (Secret Room), through the door to the secret room.")
        userInput = requestString("Choose a direction")
        userInput = parse(userInput)
        return userInput
    else:
